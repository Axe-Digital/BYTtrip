...
/*
 ...
 CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(100) NOT NULL, description TEXT);
 
 INSERT INTO  categories (title) VALUES ('plat'), ('dessert');
 
 CREATE TABLE recipes (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(150) NOT NULL, slug VARCHAR(50) NOT NULL UNIQUE, content TEXT, categories_id INTERGER, FOREIGN KEY (categories_id) REFERENCES categories (id) ON DELETE SET NULL);
 
 
 INSERT INTO  recipes (  title, slug, categories_id ) VALUES ('Salade de fruit','Salade-de-lfruit', 2), ('creme anglaise','creme_anglmaise_2', 1), ('soupe', 'sodupe', 2);
 
 DELETE FROM recipes WHERE categories_id = 2;
 
 -- UPDATE recipes SET slug='sodupe6' WHERE id=6;
 
 SELECT r.title, r.id, c.title, C.id AS categories  FROM recipes r  JOIN categories c ON r.categories_id = c.id;
...
CREATE TABLE IF NOT EXISTS synthese (
  ...
  created TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (responses_id) REFERENCES responses(responses_id),
